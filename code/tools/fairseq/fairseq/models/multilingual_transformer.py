...
from fairseq.models.transformer_no_emb import (
    TransformerNoEmbModel,
    TransformerNoEmbEncoder,
    TransformerNoEmbDecoder,
)

@register_model('multitask_transformer')
class MultitaskTransformerModel(FairseqMultiModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--no_embedding', action='store_true',
                            help='if the input is directly meaningful numbers, skip the nn.Embedding layer')
    ...
    def build_model(cls, args, task):
        ...
        return MultitaskTransformerModel(encoders, decoders)
...
@register_model('multitask_transformer_no_emb')
class MultitaskTransformerNoEmbModel(FairseqMultiModel):
    def __init__(self, encoders, decoders):
        super().__init__(encoders, decoders)
        ...
        def get_encoder(lang):
            if lang not in lang_encoders:
                if shared_encoder_embed_tokens is not None:
                    ...
                    encoder_embed_tokens = TransformerNoEmbModel.build_embedding(
                        args, task.dicts[lang], args.encoder_embed_dim, args.encoder_embed_path
                    )
                lang_encoders[lang] = TransformerNoEmbModel.build_encoder(
        ...
        def get_decoder(lang):
            if lang not in lang_decoders:
                if shared_decoder_embed_tokens is not None:
                    ...
                    decoder_embed_tokens = TransformerNoEmbModel.build_embedding(
                        args, task.dicts[lang], args.decoder_embed_dim, args.decoder_embed_path
                    )
                lang_decoders[lang] = TransformerNoEmbModel.build_decoder(
...
@register_model_architecture('multitask_transformer_no_emb', 'multitask_transformer_no_emb')
def base_multilingual_architecture(args):
    base_architecture(args)
    args.share_encoder_embeddings = getattr(args, 'share_encoder_embeddings', True)
    args.share_decoder_embeddings = getattr(args, 'share_decoder_embeddings', False)
